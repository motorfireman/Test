pipeline {
    ...
    environment {
        ...
        SONARQUBE_TOKEN = 'squ_0ba41bba3e923e47c4af1dcbf0671237941aa0a2'
		...
		stage('UI Testing') {
			steps {
				script {
					...
					sh 'curl -s -X POST -F "search=ValidTerm123" http://127.0.0.1:5000/result | grep "ValidTerm123" || echo "Valid search term test failed"'
